...
$supplier_db_fields = $this->db->list_fields('tblsuppliers');
...
if($this->input->post('download_sample') === 'true'){
    ...
    foreach($supplier_contacts_fields as $field){
        if(in_array($field,$not_importable)){continue;}
        echo '"'.ucfirst($field).'",';
        $_total_sample_fields++;
    }
    foreach($supplier_db_fields as $field){
        if(in_array($field,$not_importable)){continue;}
        echo '"'.ucfirst($field).'",';
        $_total_sample_fields++;
    }
                    ...
                    <div class="panel-body">
                            ...
                            <div class="table-responsive no-dt">
                                <table class="table table-hover table-bordered">
                                    ...
                                    <tr>
                                        <?php
                                        $total_fields = 0; ?>
                                        <?php foreach($supplier_contacts_fields as $field){
                                            if(in_array($field,$not_importable)){continue;}
                                            ?>
                                            <?php $total_fields++; ?>
                                        ...
                                        <?php } ?>
                                        <?php foreach($supplier_db_fields as $field){
                                            if(in_array($field,$not_importable)){continue;}
                                            ?>
                                            <?php $total_fields++; ?>
                        ...
                        <?php if(isset($simulate) > 0) { ?>
                            ...
                            <div class="table-responsive no-dt">
                                <table class="table table-hover table-bordered">
                                    ...
                                    <tr>
                                        <?php
                                        $supplier_db_fields = array_merge($supplier_contacts_fields,$supplier_db_fields);
                                        foreach($supplier_db_fields as $field){
                                            if(in_array($field,$not_importable)){continue;}
                                            ?>
                                            <th class="bold"><?php echo str_replace('_',' ',ucfirst($field)); ?></th>
                                        <?php } ?>
                                        <?php $custom_fields = get_custom_fields('suppliers');
                                        foreach($custom_fields as $field){ ?>
                                            <th class="bold"><?php echo $field['name']; ?></th>
                                        <?php } ?>
                                    </tr>
                        ...
                        <div class="row">
                            <div class="col-md-4 mtop15">
                                ...
                                <?php echo form_hidden('suppliers_import','true'); ?>
                                <?php echo render_input('file_csv','choose_csv_file','','file'); ?>
                                <?php echo render_select('groups_in[]',$groups,array('id','name'),'customer_groups',array(),array('multiple'=>true,'data-none-selected-text'=>_l('dropdown_non_selected_tex'))); ?>
                                <?php echo render_input('default_pass_all','default_pass_suppliers_import'); ?>
