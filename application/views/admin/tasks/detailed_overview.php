          ...
          <hr />
            ...
            <div class="row">
              <?php if(has_permission('tasks','','create')){ ?>
                ...
                <div class="col-md-3 text-center border-right" style="margin-top:-5px;">
                ...
                <div class="col-md-3 text-center" style="margin-top:-5px;">
                  <div class="form-group">
                    <label><?php echo _l('task_filter_detailed_show_tasks'); ?></label>
                    <div class="clearfix"></div>
                    <div class="radio radio-primary radio-inline">
                      <input type="radio" id="filter_all_tasks" name="show_tasks" value="" <?php if(!$this->input->post('show_tasks')){echo 'checked';} ?>>
                      <label for="filter_all_tasks"><?php echo _l('task_list_all'); ?></label>
                    </div>
                    <div class="radio radio-primary radio-inline">
                      <input type="radio" id="filter_finished" name="show_tasks" value="1" <?php if($this->input->post('show_tasks') == '1'){echo 'checked';} ?>>
                      <label for="filter_finished"><?php echo _l('task_finished'); ?></label>
                    </div>
                    <div class="radio radio-primary radio-inline">
                      <input type="radio" id="filter_unfinished" name="show_tasks" value="0" <?php if($this->input->post('show_tasks') == '0'){echo 'checked';} ?>>
                      <label for="filter_unfinished"><?php echo _l('task_list_unfinished'); ?></label>
                    </div>
                  </div>
                </div>
                <div class="col-md-2">
              ...
              <div class="panel-body">
                ...
                <div class="table-responsive">
                  <table class="table tasks-overview">
                    <thead>
                      <tr>
                        <th width="70%"><?php echo _l('tasks_dt_name'); ?></th>
                    ...
                    <tbody>
                     ...
                     foreach($data as $task){
                      ...
                      $row_class = ($task['finished'] == 1 ? 'task-finished-table-green' : 'task-unfinished-table text-danger');
                      ?>
                      <tr class="<?php echo $row_class; ?>">
                        <td class="stripped-table-data" width="70%"><a href="#" onclick="init_task_modal(<?php echo $task['id']; ?>); return false;"><?php echo $task['name']; ?></a>
                          <div class="pull-right">
                            ...
                            <span class="label label-default-light pull-left mright5" data-toggle="tooltip" data-title="<?php echo mb_substr(_l('task_total_logged_time'),0,-1); ?>">
                             <i class="fa fa-clock-o"></i> <?php echo format_seconds($task_total_logged_time_by_user); ?>
                           </span>
                          ...
                          <?php
                           ...
                           $where_other = array('taskid'=>$task['id']);
                           if(is_numeric($staff_id)){
                              $where_other['staffid'] = $staff_id;
                           }
                           ?>
                           <i class="fa fa-paperclip"></i> <?php echo total_rows('tblstafftasksattachments',$where_other); ?>
                           </span>
                           <span class="label label-default-light pull-left" data-toggle="tooltip" data-title="<?php echo _l('tasks_total_comments'); ?>">
                             <i class="fa fa-comments"></i> <?php echo total_rows('tblstafftaskcomments',$where_other); ?>
                           </span>
                         </div>
                       </td>
                       <td>
                        <?php
                        if(date('Y-m-d',strtotime($task['datefinished'])) > $task['duedate'] && $task['finished'] == 1 && is_date($task['duedate'])){
                          $finished_on_time_class = 'text-danger';
                          $finished_showcase = _l('task_not_finished_on_time_indicator');
                        } else if(date('Y-m-d',strtotime($task['datefinished'])) <= $task['duedate'] && $task['finished'] == 1 && is_date($task['duedate'])){
                          $finished_on_time_class = 'text-success';
                ...
                <tfoot>
                 <tr>
                   <td colspan="5" class="bold td-border-left-transparent"><?php echo _l('task_total_logged_time'); ?> <?php echo format_seconds($total_logged_time); ?></td>
                 </tr>
               </tfoot>
             </table>
           </div>
