...
<div class="panel_s no-padding">
    <div class="panel-body">
        ...
        <p>
            <a href="<?php echo admin_url('supplier_invoices/list_invoice/'.$_inv->id); ?>" target="_blank"><?php echo format_invoice_number($_inv->id); ?></a> - <?php echo format_money($_inv->total,$_inv->symbol); ?>
...
<?php echo form_hidden('_attachment_sale_type','supplier_invoice'); ?>
        ...
        <div class="panel-body padding-17">
            ...
            <ul class="nav nav-tabs no-margin" role="tablist">
                <li role="presentation" class="active">
                    <a href="#tab_supplier_invoice" aria-controls="tab_supplier_invoice" role="tab" data-toggle="tab">
                    <?php echo _l('invoice'); ?>
                    </a>
                </li>
    ...
    <div class="panel_s">
        <div class="panel-body">
                ...
                <div class="col-md-9 _buttons">
                    ...
                    <div class="pull-right">
                        <?php
                            ...
                            if($invoice->sent == 1){
                             $_tooltip = _l('invoice_already_send_to_supplier_tooltip',time_ago($invoice->datesend));
                            }
                            ?>
                        <a href="<?php echo admin_url('supplier_invoices/invoice/'.$invoice->id); ?>" data-toggle="tooltip" title="<?php echo _l('edit_invoice_tooltip'); ?>" class="btn btn-default pull-left mright5 btn-with-tooltip" data-placement="bottom"><i class="fa fa-pencil-square-o"></i></a>
                        <a href="<?php echo admin_url('supplier_invoices/pdf/'.$invoice->id.'?print=true'); ?>" target="_blank" class="btn pull-left btn-default mright5 btn-with-tooltip" data-toggle="tooltip" title="<?php echo _l('print'); ?>" data-placement="bottom"><i class="fa fa-print"></i></a>
                        <a href="<?php echo admin_url('supplier_invoices/pdf/'.$invoice->id); ?>" class="btn btn-default pull-left btn-with-tooltip mright5" data-toggle="tooltip" title="<?php echo _l('view_pdf'); ?>" data-placement="bottom"><i class="fa fa-file-pdf-o"></i></a>
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                <li><a href="<?php echo site_url('viewinvoice/' . $invoice->id . '/' .  $invoice->hash) ?>" target="_blank"><?php echo _l('view_invoice_as_supplier_tooltip'); ?></a></li>
                                <li>
                                    <?php if($invoice->status == 4){ ?>
                                    <a href="<?php echo admin_url('supplier_invoices/send_overdue_notice/'.$invoice->id); ?>"><?php echo _l('send_overdue_notice_tooltip'); ?></a>
                                    <?php } ?>
                                </li>
                                <li>
                                ...
                                <li>
                                    <a href="<?php echo admin_url('supplier_invoices/copy/'.$invoice->id); ?>"><?php echo _l('invoice_copy'); ?></a>
                                </li>
                                <?php } ?>
                                <?php if($invoice->sent == 0){ ?>
                                <li>
                                    <a href="<?php echo admin_url('supplier_invoices/mark_as_sent/'.$invoice->id); ?>"><?php echo _l('invoice_mark_as_sent'); ?></a>
                                </li>
                                <?php } ?>
                                <?php if(has_permission('supplier_invoices','','edit') || has_permission('supplier_invoices','','create')){ ?>
                                <li>
                                    <?php if($invoice->status != 5 && $invoice->status != 2){ ?>
                                    <a href="<?php echo admin_url('supplier_invoices/mark_as_cancelled/'.$invoice->id); ?>"><?php echo _l('invoice_mark_as',_l('invoice_status_cancelled')); ?></a>
                                    <?php } else if($invoice->status == 5) { ?>
                                    <a href="<?php echo admin_url('supplier_invoices/unmark_as_cancelled/'.$invoice->id); ?>"><?php echo _l('invoice_unmark_as',_l('invoice_status_cancelled')); ?></a>
                                    <?php } ?>
                                </li>
                                <?php } ?>
                                <?php
                                    if((get_option('delete_only_on_last_invoice') == 1 && is_last_invoice($invoice->id)) || (get_option('delete_only_on_last_invoice') == 0)){ ?>
                                <?php if(has_permission('supplier_invoices','','delete')){ ?>
                                <li data-toggle="tooltip" data-title="<?php echo _l('delete_invoice_tooltip'); ?>">
                                    <a href="<?php echo admin_url('supplier_invoices/delete/'.$invoice->id); ?>" class="text-danger delete-text _delete"><?php echo _l('delete_invoice'); ?></a>
                                </li>
                                <?php } ?>
                                <?php } ?>
                            </ul>
            ...
            <div class="tab-content">
                <div role="tabpanel" class="tab-pane ptop10 active" id="tab_supplier_invoice">
                    <div id="invoice-preview" class="mtop30">
                        <div class="col-md-12">
                            <div class="row">
                                <div class="col-md-6">
                                    <h4 class="bold"><a href="<?php echo admin_url('supplier_invoices/supplier_invoice/'.$invoice->id); ?>"><?php echo format_invoice_number($invoice->id); ?></a></h4>
                                    <address>
                                ...
                                <div class="col-sm-6 text-right">
                                    ...
                                    <address>
                                        <span class="bold"><a href="<?php echo admin_url('suppliers/supplier/'.$invoice->supplier->supplierid); ?>" target="_blank"><?php echo $invoice->supplier->company; ?></a></span><br>
                                        ...
                                        <?php
                                            ...
                                            if(!empty($invoice->supplier->vat)){
                                                echo '<br /><b>'._l('invoice_vat') .'</b>: '. $invoice->supplier->vat;
                                            }
                                            // check for customer custom fields which is checked show on pdf
                                            $pdf_custom_fields = get_custom_fields('suppliers',array('show_on_pdf'=>1));
                                            if(count($pdf_custom_fields) > 0){
                                                echo '<br />';
                                                foreach($pdf_custom_fields as $field){
                                                    $value = get_custom_field_value($invoice->supplierid,$field['id'],'suppliers');
                                                    if($value == ''){continue;}
                                    ...
                                    <p>
                                        ...
                                        <?php
                                            ...
                                            foreach($pdf_custom_fields as $field){
                                            $value = get_custom_field_value($invoice->id,$field['id'],'supplier_invoice');
                                            if($value == ''){continue;} ?>
                                        <br /><span class="mtop20">
                            ...
                            <div class="mbot15 row inline-block full-width" data-attachment-id="<?php echo $attachment['id']; ?>">
                                ...
                                <div class="col-md-4 text-right">
                                    <?php if($attachment['visible_to_supplier'] == 0){
                                        $icon = 'fa-toggle-off';
                                        $tooltip = _l('show_to_supplier');
                                        } else {
                                        $icon = 'fa-toggle-on';
                                        $tooltip = _l('hide_from_supplier');
                                        }
                                        ?>
                            ...
                            <div class="col-md-12 no-padding">
                                ...
                                <div class="row col-md-12">
                                    <?php include_once(APPPATH . 'views/admin/supplier_invoices/invoice_payments_table.php'); ?>
                                </div>
                                <?php } else { ?>
                ...
                <div role="tabpanel" class="tab-pane" id="tab_reminders">
                    ...
                    <?php $this->load->view('admin/includes/modals/reminder',array('id'=>$invoice->id,'name'=>'supplier_invoice','members'=>$members,'reminder_title'=>_l('invoice_set_reminder_title'))); ?>
                </div>
                ...
                <div role="tabpanel" class="tab-pane ptop10" id="tab_views">
                    <?php
                        $views_activity = get_views_tracking('supplier_invoice',$invoice->id);
...
<?php $this->load->view('admin/supplier_invoices/invoice_send_to_supplier'); ?>
<script>
    ...
    init_rel_tasks_table(<?php echo $invoice->id; ?>,'supplier_invoice');
</script>
