...
<!--            <a href="--><?php //echo admin_url('supplier_projects/view_project_as_supplier/'.$project->id .'/'.$project->clientid); ?><!--" target="_blank" class="pull-right">--><?php //echo _l('project_view_as_client'); ?><!--</a>-->
<!--            --><?php //} ?>
        </div>
            ...
            <div class="row">
                <div class="col-md-6">
                    <table class="table table-borded no-margin">
                        <tbody>
                        <?php if(has_permission('suppliers','','view') || is_customer_admin($project->supplierid) || has_permission('projects','','create') || has_permission('projects','','edit')){ ?>
                            <tr>
                                <td class="bold"><?php echo _l('project_supplier'); ?></td>
                                <td><a href="<?php echo admin_url(); ?>suppliers/supplier/<?php echo $project->supplierid; ?>"><?php echo $project->supplier_data->company; ?></a>
                                </td>
                            </tr>
                        <?php } ?>
                            ...
                            <tr>
                                ...
                                <td><?php echo format_seconds($this->supplier_projects_model->total_logged_time($project->id)); ?></td>
                            </tr>
                            <?php } ?>
        ...
        <div class="row">
            <div class="col-md-12">
                <?php if($project->billing_type == 2){
                    $seconds = $this->supplier_projects_model->total_logged_time($project->id);
                    $total = $this->supplier_projects_model->calculate_total_by_project_hourly_rate($seconds,$project->project_rate_per_hour);
                    $hours = $total['hours'];
                ...
                <div class="row">
                    ...
                    <div class="col-md-3">
                        <?php
                            $billable_tasks = $this->supplier_projects_model->get_tasks($project->id,array('billable'=>1));
                            $seconds = 0;
                    ...
                    <div class="col-md-3">
                        <?php
                            $billed_tasks = $this->supplier_projects_model->get_tasks($project->id,array('billable'=>1,'billed'=>1));
                            $seconds = 0;
                    ...
                    <div class="col-md-3">
                        <?php
                            $unbilled_tasks = $this->supplier_projects_model->get_tasks($project->id,array('billable'=>1,'billed'=>0));
                            $seconds = 0;
                ...
                <div class="row">
                    <div class="col-md-3">
                        <?php
                            $total = $this->supplier_projects_model->calculate_total_by_task_hourly_rate($tasks);
                    ...
                    <div class="col-md-3">
                        <?php
                            $tasks = $this->supplier_projects_model->get_tasks($project->id,array('billable'=>1));
                            $total = $this->supplier_projects_model->calculate_total_by_task_hourly_rate($tasks);
                            $total_seconds = $total['total_seconds'];
                            $total_money = $total['total_money'];
                            ?>
                    ...
                    <div class="col-md-3">
                        <?php
                            $tasks = $this->supplier_projects_model->get_tasks($project->id,array('billable'=>1,'billed'=>1));
                            $total = $this->supplier_projects_model->calculate_total_by_task_hourly_rate($tasks);
                            $total_seconds = $total['total_seconds'];
                            $total_money = $total['total_money'];
                            ?>
                    ...
                    <div class="col-md-3">
                        <?php
                            $tasks = $this->supplier_projects_model->get_tasks($project->id,array('billable'=>1,'billed'=>0));
                            $total = $this->supplier_projects_model->calculate_total_by_task_hourly_rate($tasks);
                            $total_seconds = $total['total_seconds'];
                            $total_money = $total['total_money'];
                            ?>
        ...
        <div class="panel-body no-radius">
            ...
            <div class="media">
                ...
                <div class="media-body">
                    <?php if(has_permission('projects','','edit') || has_permission('projects','','create')){ ?>
                    <a href="<?php echo admin_url('supplier_projects/remove_team_member/'.$project->id.'/'.$member['staff_id']); ?>" class="pull-right text-danger _delete"><i class="fa fa fa-times"></i></a>
                    <?php } ?>
...
<div class="row">
    ...
    <div class="col-md-6 team-members">
        ...
        <div class="panel-body no-radius">
                ...
                <div class="media"><?php foreach($contacts as $contact)
                    { ?>

                    <div class="media-left">

                            <?php echo supplier_contact_profile_image_url1($project->contactid,array('staff-profile-image-small')); ?>
...
<div class="modal fade" id="add-edit-members" tabindex="-1" role="dialog">
    <div class="modal-dialog">
        <?php echo form_open(admin_url('supplier_projects/add_edit_members/'.$project->id)); ?>
...
<div class="modal fade" id="add-edit-contacts" tabindex="-1" role="dialog">
    <div class="modal-dialog">
        <?php echo form_open(admin_url('supplier_projects/add_edit_contacts/'.$project->id)); ?>
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title"><?php echo _l('project_members'); ?></h4>
            ...
            <div class="modal-body">
                <?php
                $selected = array();
                if(isset($supplier_contacts)){
                    foreach($supplier_contacts as $c)
                    {
                        array_push($selected,$c['id']);
                    }
                }

                echo render_select('contactid',$supplier_contacts, array('id',array('firstname', 'lastname')),'Contacts',$selected,array( ));
                ?>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal"><?php echo _l('close'); ?></button>
