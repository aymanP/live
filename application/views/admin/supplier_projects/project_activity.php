           ...
           <div class="text">
              <?php if($activity['staff_id'] != 0){
                ...
                <?php } else if($activity['contact_id'] != 0){
                    $fullname = '<span class="label label-info inline-block mbot5">'._l('is_customer_indicator') . '</span> ' . get_supplier_contact_full_name($activity['contact_id']) . ' - ';
                    ?>
                    <a href="<?php echo admin_url('suppliers/supplier/'.$activity['contact_id']); ?>">
                        <img src="<?php echo supplier_contact_profile_image_url($activity['contact_id']); ?>" class="staff-profile-xs-image pull-left mright10">
                    </a>
