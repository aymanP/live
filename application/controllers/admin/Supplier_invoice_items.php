...
class Supplier_invoice_items extends Admin_controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('supplier_invoice_items_model');
    ...
    public function manage()
    {
        if (is_admin()) {
            if ($this->input->post()) {
                ...
                if ($data['itemid'] == '') {
                    $success = $this->supplier_invoice_items_model->add($data);
                    $message = '';
                    if ($success) {
                    ...
                    $success = $this->supplier_invoice_items_model->edit($data);
                    $message = '';
                    if ($success) {
    ...
    public function delete($id)
    {
        ...
        $response = $this->supplier_invoice_items_model->delete($id);
    ...
    {
        if ($this->input->is_ajax_request()) {
            echo json_encode($this->supplier_invoice_items_model->get($id));
        }
    }
    /* Get all items */
    ...
    {
        if ($this->input->is_ajax_request()) {
            echo json_encode($this->supplier_invoice_items_model->get_all_items_ajax());
        }
    }
}
