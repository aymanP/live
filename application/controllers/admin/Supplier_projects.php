...
class Supplier_projects extends Admin_controller
{
    ...
    public function index($supplierid = '')
    {
        if ($this->input->is_ajax_request()) {
                ...
                $this->perfex_base->get_table_data('supplier_projects', array(
                    'supplierid' => $supplierid,
                    'supplierid !=' => 0
                ));
            }
        $data['statuses'] = $this->supplier_projects_model->get_project_statuses();
        $data['title'] = _l('projects');
        $this->load->view('admin/supplier_projects/manage', $data);
    }
        ...
        if ($this->input->post()) {
            ...
            if ($id) {
                ...
                echo json_encode(array(
                    'url' => admin_url('supplier_projects/view/'.$this->input->post('project_id').'/?group=project_expenses'),
                    'expenseid' => $id
                ));
                die;
            }
    ...
    public function project($id = '')
    {
        ...
        if ($this->input->post())
        {
            ...
            if ($id == '') {
                ...
                $id = $this->supplier_projects_model->add($this->input->post());

                if ($id) {
                    //$this->projects_model->update_contacts($id, $contact_ids);

                    set_alert('success', _l('added_successfuly', _l('project')));
                    redirect(admin_url('supplier_projects/view/' . $id));
                }
            } else {
                if (!has_permission('projects', '', 'edit')) {
                    acccess_danied('Projects');
                }
                $success = $this->supplier_projects_model->update($this->input->post(), $id);
                $this->supplier_projects_model->update_contacts($id, $contact_ids);
        ...
        if ($id == '') {
            ...
            $data['project']         = $this->supplier_projects_model->get($id);
            $data['project_members'] = $this->supplier_projects_model->get_project_members($id);
            $title                   = _l('edit', _l('project_lowercase'));

            $supplier_id = $data['project']->supplierid;

            $data['project_contacts'] = $this->supplier_projects_model->get_project_contacts($id);
            $data['contacts']        = $this->supplier_model->get_contacts($supplier_id);
        }
        ...
        $data['contacts']        = $this->supplier_model->get_contacts($supplier_id);
        $data['last_project_settings'] = $this->supplier_projects_model->get_last_project_settings();
        $data['settings']              = $this->supplier_projects_model->get_settings();
        $data['statuses'] = $this->supplier_projects_model->get_project_statuses();
        $data['staff']     = $this->staff_model->get('', 1);
        $data['suppliers'] = $this->supplier_model->get('', 1);
        $data['title']     = $title;
        $this->load->view('admin/supplier_projects/project', $data);
    }
    public function view($id)
    {
        if ($this->supplier_projects_model->is_member($id) || has_permission('projects', '', 'view')) {
            $project = $this->supplier_projects_model->get($id);
            if (!$project) {
...
//            $this->load->model('estimates_model');
            ...
            $data['currency']                  = $this->supplier_projects_model->get_currency($id);
...
//            ));
            ...
            $data['tasks'] = $this->supplier_projects_model->get_tasks($id);
            $this->load->model('payment_modes_model');
            $data['payment_modes'] = $this->payment_modes_model->get('', true);
            $data['members']       = $this->supplier_projects_model->get_project_members($id);
            $data['contacts']       = $this->supplier_projects_model->get_project_cont($data['project']->contactid);
            $data['supplier_contacts']= $this->supplier_model->get_contacts($data['project']->supplierid);
            $data['staff']         = $this->staff_model->get('', 1);
            $data['gantt_data']    = $this->supplier_projects_model->get_gantt_data($id);
            $data['title']         = $data['project']->name;
            $data['invoiceid']     = '';
//            $data['estimateid']     = '';
            $data['status']        = '';
            $data['custom_view']   = '';
            $data['files']         = $this->supplier_projects_model->get_files($id);
            $percent               = $this->supplier_projects_model->calc_progress($id);
            ...
            if ($this->input->get('discussion_id')) {
                ...
                $data['discussion']                        = $this->supplier_projects_model->get_discussion($this->input->get('discussion_id'), $id);
                $data['current_user_is_admin']             = is_admin();
            }
            $data['activity']              = $this->supplier_projects_model->get_activity($id);
            $data['bodyclass']             = 'project';
            $data['staff_notes']           = $this->supplier_projects_model->get_staff_notes($id);
            ...
            $data['group_view']            = $this->load->view('admin/supplier_projects/' . $view, $data, true);
            $data['percent']               = $percent;
            $data['projects_assets']       = true;
            $data['circle_progress_asset'] = true;

            $this->load->view('admin/supplier_projects/view', $data);
        } else {
    ...
    public function update_task_milestone(){
        if($this->input->post()){
            $this->supplier_projects_model->update_task_milestone($this->input->post());
        }
    }
    public function pin_action($project_id){
        $this->supplier_projects_model->pin_action($project_id);
        redirect($_SERVER['HTTP_REFERER']);
    }
    public function add_edit_members($project_id)
    {
        if (has_permission('projects', '', 'edit') || has_permission('projects','','create')) {
            $this->supplier_projects_model->add_edit_members($this->input->post(), $project_id);
            redirect($_SERVER['HTTP_REFERER']);
        }
    }
    ...
    {
        if (has_permission('projects', '', 'edit') || has_permission('projects','','create')) {
            $this->supplier_projects_model->add_edit_contacts($this->input->post('contactid'),$project_id);//add_edit_members($this->input->post(), $project_id);
            redirect($_SERVER['HTTP_REFERER']);
        }
    }
//    public function discussions($project_id)
//    {
//        if ($this->supplier_projects_model->is_member($project_id) || has_permission('projects', '', 'view')) {
//            if ($this->input->is_ajax_request()) {
...
//    }
    public function change_milestone_color(){
        if($this->input->post()){
            $this->supplier_projects_model->update_milestone_color($this->input->post());
        }
    }
    ...
    {
        if ($this->input->is_ajax_request()) {
            $this->supplier_projects_model->change_file_visibility($id, $visible);
        }
    }
    public function change_activity_visibility($id, $visible)
    {
        if (has_permission('projects','','create')) {
            if ($this->input->is_ajax_request()) {
                $this->supplier_projects_model->change_activity_visibility($id, $visible);
            }
        }
    }
    public function remove_file($project_id, $id)
    {
        $this->supplier_projects_model->remove_file($id);
        redirect(admin_url('supplier_projects/view/' . $project_id . '?group=project_files'));
    }
    ...
    public function milestone($id = '')
    {
        if ($this->input->post()) {
            ...
            $success = false;
            if (!$this->input->post('id')) {
                $id = $this->supplier_projects_model->add_milestone($this->input->post());
                if ($id) {
                ...
                $id   = $data['id'];
                unset($data['id']);
                $success = $this->supplier_projects_model->update_milestone($data, $id);
                if ($success) {
    ...
    {
        if (has_permission('projects', '', 'delete')) {
            if ($this->supplier_projects_model->delete_milestone($id)) {
                set_alert('deleted', 'project_milestone');
            }
        }
        redirect(admin_url('supplier_projects/view/' . $project_id . '?group=project_milestones'));
    }
    ...
    public function timesheet_task_assignees($task_id, $project_id, $staff_id = 'undefined')
    {
        ...
        foreach ($assignees as $staff) {
            ...
            if ($staff['assigneeid'] == $staff_id && $this->supplier_projects_model->is_member($project_id, $staff_id)) {
                $selected = ' selected';
            }
    ...
    {
        if (has_permission('projects', '', 'edit') || has_permission('projects', '', 'create')) {
            if ($this->supplier_projects_model->remove_team_member($project_id, $staff_id)) {
                set_alert('success', _l('project_member_removed'));
            }
        }
        redirect(admin_url('supplier_projects/view/' . $project_id));
    }
    public function save_note($project_id)
    {
        if ($this->input->post()) {
            $success = $this->supplier_projects_model->save_note($this->input->post(), $project_id);
            if ($success) {
                set_alert('success', _l('updated_successfuly', _l('project_note')));
            }
            redirect(admin_url('supplier_projects/view/' . $project_id . '?group=project_notes'));
        }
    }
    public function delete($project_id)
    {
        if (has_permission('projects', '', 'delete')) {
            $project = $this->supplier_projects_model->get($project_id);
            $success = $this->supplier_projects_model->delete($project_id);
            if ($success) {
                set_alert('success', _l('deleted', _l('project')));
                redirect(admin_url('supplier_projects'));
            } else {
    ...
    {
        if (has_permission('projects', '', 'create')) {
            $id = $this->supplier_projects_model->copy($project_id);
            if ($id) {
    ...
    public function mass_stop_timers($project_id, $billable = 'false')
    {
        if (has_permission('invoices', '', 'create')) {
            ...
            $tasks                = $this->supplier_projects_model->get_tasks($project_id, $where);
    ...
    {
        if (has_permission('invoices', '', 'create')) {
            $data['billable_tasks']     = $this->supplier_projects_model->get_tasks($project_id, array(
                'billable' => 1,
                'billed' => 0,
                'startdate <=' => date('Y-m-d')
            ));
            $data['not_billable_tasks'] = $this->supplier_projects_model->get_tasks($project_id, array(
                'billable' => 1,
                'billed' => 0,
                'startdate >' => date('Y-m-d')
            ));
            ...
            $this->load->view('admin/supplier_projects/project_pre_invoice_settings', $data);
        }
    }
        ...
        if (has_permission('invoices', '', 'create')) {
            ...
            if ($tasks) {
                ...
                if ($type == 'single_line') {
                    ...
                    $timesheets     = $this->supplier_projects_model->get_timesheets($project_id, $tasks);
            ...
            $data['do_not_auto_toggle'] = true;
            $data['supplier_id'] = $project->supplierid;
            $data['invoice_from_project'] = true;
            $data['add_items'] = $items;
            $this->load->view('admin/supplier_projects/invoice_project', $data);
        }
    }
    public function get_rel_project_data($id, $task_id = '')
    {
        if ($this->input->is_ajax_request()) {
            ...
            echo json_encode(array(
                ...
                'milestones' => render_select('milestone', $this->supplier_projects_model->get_milestones($id), array(
                    'id',
                    'name'
    ...
    public function invoice_project($project_id)
    {
        if (has_permission('invoices', '', 'create')) {
            ...
            $data = $this->input->post();
            $data['project_id'] = $project_id;
            $invoice_id = $this->supplier_invoices_model->add($data);
            if ($invoice_id) {
                $this->supplier_projects_model->log_activity($project_id, 'project_activity_invoiced_project', format_invoice_number($invoice_id));
                set_alert('success', _l('project_invoiced_successfuly'));
            }
            redirect(admin_url('supplier_projects/view/' . $project_id . '?group=project_invoices'));
        }
    }
//    public function view_project_as_client($id, $supplierid)
//    {
...
//            $this->clients_model->login_as_client($supplier_id);
//            redirect(site_url('clients/project/' . $id));
//        }
//    }

    public function get_contacts_by_supplier($id)
    {
        $contacts = $this->supplier_model->get_contacts($id);
