...
class Supplier_projects_model extends CRM_Model
{
    ...
    public function get_currency($id)
    {
        ...
        $supplier_currency = $this->supplier_model->get_supplier_default_currency($project->supplierid);
        if ($supplier_currency != 0) {
            $currency = $this->currencies_model->get($supplier_currency);
        } else {
    ...
    public function get_supplier_projects($contact_id, $where)
    {
        $this->db->where($where)
    ...
    public function get($id = '', $where = array())
    {
        ...
        if (is_numeric($id)) {
            ...
            if ($project) {
                ...
                $project->supplier_data = new StdClass();
                $project->supplier_data = $this->supplier_model->get($project->supplierid);
                return $project;
            }
            return null;
        }
...
//        }
        ...
        if ($file) {
            if(file_exists(PROJECT_ATTACHMENTS_FOLDER . $file->project_id . '/' . $file->file_name)){
                if (unlink(PROJECT_ATTACHMENTS_FOLDER . $file->project_id . '/' . $file->file_name)) {
                    ...
                    $this->log_activity($file->project_id, 'project_activity_project_file_removed', $file->file_name, $file->visible_to_supplier);
                }
            }
    ...
    public function add_milestone($data)
    {
        ...
        if ($insert_id) {
            ...
            $this->log_activity($milestone->project_id, 'project_activity_created_milestone', $milestone->name, $show_to_supplier);
    ...
    public function update_milestone($data, $id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->log_activity($milestone->project_id, 'project_activity_updated_milestone', $milestone->name, $show_to_supplier);
    ...
    public function delete_milestone($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->log_activity($milestone->project_id, 'project_activity_deleted_milestone', $milestone->name, $show_to_supplier);
    ...
    public function get_projects_for_ticket($supplier_id){
        return $this->get('',array('supplierid'=>$supplier_id));
    }
            ...
            if (isset($data['discussions'])) {
                ...
                foreach ($discussions as $discussion) {
                    $this->db->insert('tblprojectdiscussions', array(
                        ...
                        'show_to_supplier' => $discussion['show_to_supplier'],
...
//        }
    ...
    public function log_activity($project_id, $description_key, $additional_data = '', $visible_to_customer = 0)
    {
        if (!DEFINED('CRON')) {
//            if (is_client_logged_in()) {
//                $data['contact_id'] = get_contact_user_id();
...
//            } else
    ...
    public function send_project_email_template($project_id,$staff_template,$supplier_template,$action_visible_to_supplier,$additional_data = array()){
        if(count($additional_data) == 0){
            $additional_data['suppliers'] = array();
...
//            }
        ...
        if($action_visible_to_supplier == 1){
...
//                }
                if (has_contact_permission('projects', $contact['id'])) {
                    $merge_fields = array();
                    $merge_fields = array_merge($merge_fields, get_supplier_contact_merge_fields($project->supplierid,$contact['id']));
                    $merge_fields = array_merge($merge_fields, get_project_merge_fields($project->id,$additional_data['suppliers']));
                    $this->emails_model->send_email_template($supplier_template,$contact['email'],$merge_fields);
                }
            }
        }
    }
